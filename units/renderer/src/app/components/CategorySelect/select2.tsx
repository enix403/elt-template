...
export class CategorySelect2 extends React.Component<IProps, IState> {
    ...
    applyNodesActions = (oldState: INodeStateInfo, actions: TreeAction[]) => {
        ...
        for (const action of actions) {
            newNodesState = CategorySelect2.treeReducer(newNodesState, action);
        }
    ...
    handleNodeClick = (node: TreeNodeInfo, nodePath: NodePath) => {
        ...
        this.setState({
            nodes: newNodesState,
            selectedNodePath: targetSelected ? nodePath : null
        });
    }

    handleNodeCollapse = (_node: TreeNodeInfo, nodePath: NodePath) => {
        const shouldDeselect = this.state.selectedNodePath && isNodeChild(nodePath, this.state.selectedNodePath);
